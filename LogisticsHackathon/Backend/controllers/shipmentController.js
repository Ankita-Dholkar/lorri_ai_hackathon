...
export const generateOTP = async (req, res) => {
  ...
  try {
    ...
    if (receiverEmail) {
      ...
      shipment.consigneeEmail = receiverEmail
    } else if (shipment.consigneeEmail) {
      if (!/^\S+@\S+\.\S+$/.test(shipment.consigneeEmail)) {
         return res.status(400).json({ message: 'Incorrect email' })
      }
    } else {
...
export const requestInvoice = async (req, res) => {
  try {
    ...
    const manualChargesSum = shipment.waitingCharges + shipment.loadingUnloadingCharges + shipment.detentionCharges
    
    shipment.fraudReasons = []
    
    // 1. Verify POD Details
    if (!shipment.deliveryTimestamp) {
       shipment.fraudReasons.push('MISSING POD: Delivery not verified by receiver')
    }

    // 2. Charge Discrepancy Analysis
    if (requestedTotal > expectedTotal) {
      shipment.riskLevel = 'High'
      shipment.fraudProbability = 70
      shipment.fraudReasons.push(`EXCESSIVE CHARGES: Requested amount (₹${requestedTotal}) exceeds Expected Total (₹${expectedTotal})`)
    } else if (manualChargesSum > 0) {
      shipment.riskLevel = 'Medium'
      shipment.fraudProbability = 40
      shipment.fraudReasons.push(`MANUAL REVIEW REQUIRED: Carrier requested ₹${manualChargesSum} in additional fees`)
    } else {
      shipment.riskLevel = 'Low'
      shipment.fraudProbability = 5 // Low base probability
    }
